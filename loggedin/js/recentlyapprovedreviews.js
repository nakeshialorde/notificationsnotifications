$(function(){
	$.ajax({
		url:"https://e-solutionsgroup.com:8080/api/COBRecentSecurityReivews",
		crossDomain:true,
		dataType:'json',
		cache:false,
		...
		success:function(data){
			for(i=0;i<data.length;i++){
				console.log(data);
				$("<div></div>")
					.addClass("recentlyApprovedReview listItem")
					.attr("recentlyApprovedReviewID",data[i].ReviewID)
					.append(
						$("<div></div>")
							.addClass("FullName")
							.append(
					...
					.appendTo("#RecentlyApprovedReviewList");
			}
		}
	});
});